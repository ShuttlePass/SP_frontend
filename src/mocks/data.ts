...
export const mockCourseService = {
  ...
  updateCourse: async (courseData: {
    ...
    const courseIndex = MOCK_ASSIGNED_COURSES.findIndex(
  ...
  checkTimeSlotAvailability: (date: string, startTime: string) => {
    const isBooked = MOCK_ASSIGNED_COURSES.some(
      (course) => course.date === date && course.startTime === startTime,
    );
  ...
  assignCourse: async (courseData: {
    ...
    const newCourse = {
      ...
      ...courseData,
      maxStudents:
        MOCK_COURSES.find((c) => c.type === courseData.type)?.maxStudents || 0,
    };
  ...
  getAvailableTimeSlots: async (date: string) => {
    // 해당 날짜의 예약된 수업들 조회
    const bookedCourses = MOCK_ASSIGNED_COURSES.filter(
      (course) => course.date === date,
    );

    return MOCK_TIME_SLOTS.map((slot) => ({
      ...slot,
      isAvailable: !bookedCourses.some(
        (course) => course.startTime === slot.startTime,
      ),
      // 선택적: 어떤 수업이 배정되어 있는지 정보 추가
      bookedInfo: bookedCourses.find(
        (course) => course.startTime === slot.startTime,
      ),
    }));
  },
